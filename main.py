...
class ResultsHandler(webapp2.RequestHandler):
    def get(self):
        if not self.request.get("location") and not self.request.get("category"):
            self.response.write('Error: Please go back and pick a location and category<form method="get" action="/"><button id=backbutton type="submit">Back</button></form>')
        else:
                ...
                for event in listOfEvents:
                    ...
                    if listOfEvents[i]["image"]is None:
                        if self.request.get('category') == 'outdoors_recreation':
                            ...
                            event_image_url_medium[listOfEvents[i]["title"]] = "/resources/other_image.jpg"
                        elif self.request.get('category') == '':
                            event_image_url_medium[listOfEvents[i]["title"]] = "/resources/other_image.jpg"
                    else:
...
class SavedEvent(webapp2.RequestHandler):
    def get(self):
        ...
        if user:
            ...
            event_obj = Event(event_id = eventId, event_title = eventTitle, event_image_url = eventImageURL, event_category = eventCategory)
